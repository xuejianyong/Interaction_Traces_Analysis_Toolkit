...
dataPath = os.path.join(scriptPath,'other')
        ...
        if lineIndex%modeParameter == 1:
            ...
            if lineStrs[0] == 'loop_number':
                if len(loopNums) == 0:
                    ...
                    loopNums.append(int(loopNumber))
                    addFlag = True
        elif lineIndex%modeParameter == 0:
            if addFlag == True:
                lineStrs = line.split(':')
...
x_ticks = np.arange(0, 1230, 100)
y_ticks = np.arange(0, 400, 30)
plt.xticks(x_ticks)
plt.yticks(y_ticks)
plt.legend()
plt.show()
